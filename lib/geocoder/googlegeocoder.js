...
GoogleGeocoder.prototype._formatResult = function(result) {
    ...
    for (var i = 0; i < result.address_components.length; i++) {
        ...
        // State
        if (result.address_components[i].types.indexOf('administrative_area_level_3') >= 0 && result.address_components[i].long_name !== 'England') {
            state = result.address_components[i].long_name;
        } else if(result.address_components[i].types.indexOf('administrative_area_level_2') >= 0 && result.address_components[i].long_name !== 'England'){
            state = result.address_components[i].long_name;
		} else if(result.address_components[i].types.indexOf('administrative_area_level_1') >= 0 && result.address_components[i].long_name !== 'England'){
            state = result.address_components[i].long_name;
		}
        if (result.address_components[i].types.indexOf('administrative_area_level_3') >= 0 && result.address_components[i].short_name !== 'England') {
            stateCode = result.address_components[i].short_name;
        } else if(result.address_components[i].types.indexOf('administrative_area_level_2') >= 0 && result.address_components[i].short_name !== 'England'){
            stateCode = result.address_components[i].short_name;
		} else if(result.address_components[i].types.indexOf('administrative_area_level_1') >= 0 && result.address_components[i].short_name !== 'England'){
            stateCode = result.address_components[i].short_name;
		}
        // City
        if (result.address_components[i].types.indexOf('locality') >= 0) {
            city = result.address_components[i].long_name;
        } else if(result.address_components[i].types.indexOf('postal_town') >= 0) {
			city = result.address_components[i].long_name;
		}
        // Address
        ...
        } else if(result.address_components[i].types.indexOf('establishment') >= 0) {
			streetNumber = result.address_components[i].long_name;
		}
    }
